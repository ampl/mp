...
IlogCPDriver::IlogCPDriver() :
   mod_(env_), gotopttype_(false), debug_(false), numberofs_(CreateVar(env_)) {
  options_[DEBUGEXPR] = 0;
  options_[OPTIMIZER] = AUTO;
  options_[TIMING] = 0;
  options_[USENUMBEROF] = 1;

  OptionInfo<IlogCPDriver> &options = Driver::options();

  options.set_solver_name("ilogcp");
  options.set_long_solver_name(fmt::Format("ilogcp {0}.{1}.{2}")
      << IloConcertVersion::_ILO_MAJOR_VERSION
      << IloConcertVersion::_ILO_MINOR_VERSION
      << IloConcertVersion::_ILO_TECH_VERSION);
  options.set_version(fmt::Format("AMPL/IBM ILOG CP Optimizer [{0} {1}.{2}.{3}]")
      << IloConcertVersion::_ILO_NAME << IloConcertVersion::_ILO_MAJOR_VERSION
      << IloConcertVersion::_ILO_MINOR_VERSION
      << IloConcertVersion::_ILO_TECH_VERSION);
  options.set_options_var_name(xxxvers);
  options.set_driver_date(YYYYMMDD);
  ...
  options.AddStrOption("alldiffinferencelevel",
  ...
  options.AddStrOption("branchlimit",
  ...
  options.AddStrOption("choicepointlimit",
  ...
  options.AddStrOption("distributeinferencelevel",
  ...
  options.AddStrOption("dynamicprobing",
  ...
  options.AddDblOption("dynamicprobingstrength",
  ...
  options.AddIntOption<int>("mipdisplay",
  ...
  options.AddStrOption("outlev", "Synonym for \"logverbosity\".",
      &IlogCPDriver::SetCPOption,
      CPOptionInfo(IloCP::LogVerbosity, IloCP::Quiet, Verbosities));

  options.AddStrOption("propagationlog",
  ...
  options.AddDblOption("restartgrowthfactor",
  ...
  options.AddStrOption("searchtype",
...
bool IlogCPDriver::ParseOptions(char **argv) {
  // Get optimizer type.
  gotopttype_ = false;
  options().DisableOptionEcho(ASL_OI_echo);
  ...
  gotopttype_ = true;
  options().EnableOptionEcho(ASL_OI_echo);
  if (!Driver::ParseOptions(argv, *this))
    return false;
...
int IlogCPDriver::Run(char **argv) {
  ...
  if (!problem.Read(argv, options()) || !ParseOptions(argv))
    return 1;
  ...
  format_message("{0}: {1}\n") << options().long_solver_name() << status;
