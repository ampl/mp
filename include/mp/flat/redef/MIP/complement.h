  ...
  void Convert(const ItemType& cc, int ) {
    ...
    if (fin_var_lb && !fin_var_ub) {
      ...
      auto res_neg_con_lb = GetMC().AssignResultVar2Args(     // TODO wrong for QuadCon
            CondLinConLE{ { expr.GetAlgConBody(), -expr.constant_term() } });
      /// res3 = (res1 \/ res2)
      ...
      GetMC().AddConstraint(
            LinConGE{ expr.GetAlgConBody(), -expr.constant_term() } );
      ...
      auto res1 = GetMC().AssignResultVar2Args(
            AndConstraint{ {
                ...
                GetMC().AssignResultVar2Args(        // TODO wrong for QuadCon
                             CondLinConLE{ { expr_neg.GetAlgConBody(),
                                             -expr_neg.constant_term() } })
                           } });
      /// res2 = (body==0)
      auto res2 = GetMC().AssignResultVar2Args(      // TODO wrong for QuadCon
            CondLinConEQ{ { expr.GetAlgConBody(), -expr.constant_term() } });
      /// res3 = (var >= ub && con <= 0)
      auto res3 = GetMC().AssignResultVar2Args(
            AndConstraint{ {
                GetMC().AssignResultVar2Args(
                             CondLinConLE{ { {{-1.0}, {compl_var}}, -var_ub } }),
                GetMC().AssignResultVar2Args(        // TODO wrong for QuadCon
                             CondLinConLE{ { expr.GetAlgConBody(),
                                             -expr.constant_term() } })
                           } });
      /// res4 = (res1 \/ res2 \/ res3)
